...
@frappe.whitelist(allow_guest = 0 )
def delivery_accepted_request(request_id , status):
	return track_delivery(request_id , "Accepted")

@frappe.whitelist(allow_guest = 0 )
def delivery_arrived(request_id , status):
	return track_delivery(request_id , "Arrived")

@frappe.whitelist(allow_guest = 0 )
def delivery_cancel_request(request_id , status):
	return track_delivery(request_id , "Delivery Cancel")

@frappe.whitelist(allow_guest = 0 )
def track_delivery(request_id , status):
	...
	if frappe.db.exists("Request Delivery",{'name':request_id}):
		request_delivery = frappe.get_doc("Request Delivery" , request_id)
		request_delivery.status = status
